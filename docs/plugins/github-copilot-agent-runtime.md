...
read_when:
  ...
  - You need configuration examples for the `github-copilot` runtime
  - You are wiring an agent to subscription Copilot (github / openclaw / copilot) and want it to run through the Copilot CLI
---

The bundled `github-copilot` extension lets OpenClaw run embedded subscription
...
- OpenClaw with the bundled `github-copilot` extension available.
- If your config uses `plugins.allow`, include `@openclaw/github-copilot`.
...
Per-agent precedence, applied during `runGithubCopilotAttempt`:
...
(`runGithubCopilotAttempt({...})`) plus a small set of env defaults inside
`extensions/github-copilot/src/`:
...
`runGithubCopilotAttempt` dual-writes each turn's mirrorable messages into the
OpenClaw audit transcript via
`extensions/github-copilot/src/dual-write-transcripts.ts`. The mirror is
per-session scoped (`github-copilot:${sessionId}`) and uses a per-message
...
`extensions/github-copilot/doctor-contract-api.ts` is auto-loaded by
...
- One `sessionRouteStateOwners` entry claiming provider `github-copilot`;
  runtime `github-copilot`; CLI session key `github-copilot`; auth profile
  prefix `github-copilot:`.

`extensions/github-copilot/src/doctor-probes.ts` exports three imperative probes
...
- PI session state is not migrated when an agent switches to `github-copilot`.
...
- **Interactive `ask_user` is not yet wired.** The SDK's
  ...
  dormant scaffolding in `extensions/github-copilot/src/user-input-bridge.ts`
...
`extensions/github-copilot/src/tool-bridge.ts` and mirrors the PI
