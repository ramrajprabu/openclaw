...
async function createTempSessionFile() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-copilot-sdk-mirror-"));
  tempDirs.push(dir);
...
describe("mirrorCopilotSdkTranscript", () => {
  it("mirrors user, assistant, and tool result messages into the OpenClaw transcript", async () => {
    ...
    await mirrorCopilotSdkTranscript({
      sessionFile,
      sessionKey: "session-1",
      messages: [userMessage, assistantMessage, toolResultMessage],
      idempotencyScope: "copilot-sdk:session-1",
    });
    ...
    expect(raw).toContain(
      `"idempotencyKey":"copilot-sdk:session-1:user:${expectedFingerprint(userMessage)}"`,
    );
    expect(raw).toContain(
      `"idempotencyKey":"copilot-sdk:session-1:assistant:${expectedFingerprint(assistantMessage)}"`,
    );
    expect(raw).toContain(
      `"idempotencyKey":"copilot-sdk:session-1:toolResult:${expectedFingerprint(toolResultMessage)}"`,
    );
  });

  it("creates the transcript directory on first mirror", async () => {
    const root = await makeRoot("openclaw-copilot-sdk-mirror-missing-dir-");
  ...
  it("runs before_message_write before appending mirrored messages", async () => {
    ...
    expect(raw).toContain(
      `"idempotencyKey":"copilot-sdk:session-1:assistant:${expectedFingerprint(sourceMessage)}"`,
    );
  });
  ...
  it("uses attached identity instead of content fingerprint when provided", async () => {
    ...
    const tagged = attachCopilotSdkMirrorIdentity(baseMessage, "sdk-session-1:assistant:0");

    await mirrorCopilotSdkTranscript({
      sessionFile,
      messages: [tagged],
      idempotencyScope: "copilot-sdk:openclaw-session-1",
    });

    const raw = await fs.readFile(sessionFile, "utf8");
    expect(raw).toContain(
      '"idempotencyKey":"copilot-sdk:openclaw-session-1:sdk-session-1:assistant:0"',
    );
  ...
  it("preserves explicit identity across attachCopilotSdkMirrorIdentity overrides", async () => {
    const sessionFile = await createTempSessionFile();
    const base = makeAgentAssistantMessage({
      content: [{ type: "text", text: "x" }],
      timestamp: Date.now(),
    });
    const first = attachCopilotSdkMirrorIdentity(base, "id-1");
    const second = attachCopilotSdkMirrorIdentity(first, "id-2");
...
describe("dualWriteCopilotSdkTranscriptBestEffort", () => {
