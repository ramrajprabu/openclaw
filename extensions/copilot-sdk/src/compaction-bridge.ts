...
// Compaction bridge for the Copilot SDK harness.
//
...
export type { SdkInfiniteSessionConfig as CopilotSdkInfiniteSessionConfig };

export interface CopilotSdkInfiniteSessionOptions {
  enabled?: boolean;
...
export function createInfiniteSessionConfig(
  options?: CopilotSdkInfiniteSessionOptions,
...
export interface OpenClawCompactionMarker {
  readonly version: 1;
  readonly source: "copilot-sdk-harness";
  readonly sessionId: string;
  readonly ts: number;
  /**
...
export async function writeOpenClawCompactionMarker(
  ...
  if (!input.workspaceDir || typeof input.workspaceDir !== "string") {
    throw new Error("[copilot-sdk:compaction-bridge] workspaceDir is required to write a marker");
  }
  if (!input.sessionId || typeof input.sessionId !== "string") {
    throw new Error("[copilot-sdk:compaction-bridge] sessionId is required to write a marker");
  }
  ...
  const marker: OpenClawCompactionMarker = compactJsonValue({
    version: 1 as const,
    source: "copilot-sdk-harness" as const,
