...
import {
  ...
  staticAnswerPolicy,
  DENY_ALL_ANSWER,
  type CopilotSdkUserInputContext,
...
function makeCtx(overrides: Partial<CopilotSdkUserInputContext> = {}): CopilotSdkUserInputContext {
  return {
    request: makeRequest(),
    sessionId: "sess-1",
    ...overrides,
...
describe("delegatingUserInputPolicy", () => {
  ...
  it("returns DENY_ALL_ANSWER when host callback returns undefined", async () => {
    const onRequest = vi.fn<CopilotSdkUserInputPolicy>().mockResolvedValue(undefined);
    const policy = delegatingUserInputPolicy({ onRequest });
    const result = await policy(makeCtx());
    expect(result).toEqual({ answer: DENY_ALL_ANSWER, wasFreeform: true });
  });
...
describe("composeUserInputPolicies", () => {
  it("returns the first non-undefined result and skips subsequent policies", async () => {
    const a: CopilotSdkUserInputPolicy = () => undefined;
    const b: CopilotSdkUserInputPolicy = () => ({ answer: "from-b", wasFreeform: true });
    const c = vi.fn<CopilotSdkUserInputPolicy>(() => ({ answer: "from-c", wasFreeform: true }));
  ...
  it("short-circuits to error-message response when any policy throws", async () => {
    const later = vi.fn<CopilotSdkUserInputPolicy>(() => ({ answer: "later", wasFreeform: true }));
    const policy = composeUserInputPolicies(() => {
...
describe("createUserInputBridge", () => {
  ...
  it("forwards the SDK sessionId into the policy context", async () => {
    const policy = vi.fn<CopilotSdkUserInputPolicy>(() => ({ answer: "x", wasFreeform: true }));
