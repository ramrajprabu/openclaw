/**
 * Hooks bridge for the copilot-sdk harness.
 *
...
export interface CopilotSdkHooksConfig {
...
const DEFAULT_HOOK_ERROR_HANDLER: NonNullable<CopilotSdkHooksConfig["onHookError"]> = ({
  hookName,
  error,
}) => {
  // eslint-disable-next-line no-console -- isolated diagnostic; subsystem logger lives outside the package boundary
  console.warn(`[copilot-sdk hooks-bridge] ${hookName} handler threw:`, error);
};
...
function isolate<TArgs extends readonly unknown[], TResult>(
  ...
  onError: NonNullable<CopilotSdkHooksConfig["onHookError"]>,
 ...
 * `CopilotSdkHooksConfig`. Returns `undefined` when no handlers were
 * supplied so the SDK skips the hook subsystem entirely.
 */
export function createHooksBridge(config?: CopilotSdkHooksConfig): SdkSessionHooks | undefined {
