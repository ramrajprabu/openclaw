/**
 * Mirrors the AgentMessages produced by the copilot-sdk harness into the
 ...
 * caller-side `dualWriteCopilotSdkTranscriptBestEffort` wrapper used
 ...
 */
export function attachCopilotSdkMirrorIdentity<T extends AgentMessage>(
  message: T,
  identity: string,
): T {
...
export interface MirrorCopilotSdkTranscriptParams {
  sessionFile: string;
  sessionKey?: string;
  agentId?: string;
   ...
   * `codex-app-server:${threadId}`; we use `copilot-sdk:${sessionId}`
...
export async function mirrorCopilotSdkTranscript(
  params: MirrorCopilotSdkTranscriptParams,
): Promise<void> {
 ...
 * this turn.
 */
export async function dualWriteCopilotSdkTranscriptBestEffort(
  params: MirrorCopilotSdkTranscriptParams,
): Promise<void> {
  try {
    await mirrorCopilotSdkTranscript(params);
  } catch (error) {
    console.warn("[copilot-sdk-attempt] dual-write transcript mirror failed", error);
  }
}
