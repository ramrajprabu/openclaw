...
// Telemetry bridge for the Copilot SDK harness.
//
// SDK surface:
...
export type { SdkTraceContext as CopilotSdkTraceContext };
export type { SdkTelemetryConfig as CopilotSdkTelemetryConfig };

export type CopilotSdkTraceContextSource = () =>
  | SdkTraceContext
  | undefined
  | Promise<SdkTraceContext | undefined>;
export type CopilotSdkTraceparentSource = () => string | undefined | Promise<string | undefined>;
export type CopilotSdkTracestateSource = () => string | undefined | Promise<string | undefined>;

export interface CopilotSdkTraceContextErrorInfo {
  readonly part: "traceContext" | "traceparent" | "tracestate";
  readonly error: Error;
}

export interface CopilotSdkTraceContextOptions {
  /**
   ...
   */
  getTraceContext?: CopilotSdkTraceContextSource;
  /**
   ...
   */
  getTraceparent?: CopilotSdkTraceparentSource;
  /**
...
function defaultOnTraceContextError(info: CopilotSdkTraceContextErrorInfo): void {
  // eslint-disable-next-line no-console -- bridge has no host logger by design.
  console.warn(`[copilot-sdk:telemetry-bridge] ${info.part} source failed: ${info.error.message}`);
}

function safeNotify(
  notifier: (info: CopilotSdkTraceContextErrorInfo) => void,
  info: CopilotSdkTraceContextErrorInfo,
): void {
  try {
    notifier(info);
  } catch {
...
export function createTraceContextProvider(
  options?: CopilotSdkTraceContextOptions,
...
export interface CopilotSdkTelemetryOptions {
  otlpEndpoint?: string;
  filePath?: string;
  exporterType?: string;
  sourceName?: string;
...
export function createTelemetryConfig(
  options?: CopilotSdkTelemetryOptions,
