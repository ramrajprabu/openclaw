 ...
 * module:
 *
 *   1. Defines a small `CopilotSdkUserInputPolicy` contract that the
...
export interface CopilotSdkUserInputContext {
 ...
 * code path.
 */
export type CopilotSdkUserInputPolicy = (
  ctx: CopilotSdkUserInputContext,
...
export const DENY_ALL_ANSWER =
  "[copilot-sdk harness: no user-input policy installed; request declined]";

export const denyAllUserInputPolicy: CopilotSdkUserInputPolicy = () => ({
 ...
 */
export const firstChoicePolicy: CopilotSdkUserInputPolicy = ({ request }) => {
...
export function staticAnswerPolicy(options: StaticAnswerPolicyOptions): CopilotSdkUserInputPolicy {
...
export interface DelegatingUserInputPolicyOptions {
   ...
   */
  onRequest: CopilotSdkUserInputPolicy;
  /**
   ...
   */
  onError?: CopilotSdkUserInputPolicy;
}
...
export function composeUserInputPolicies(
  ...policies: CopilotSdkUserInputPolicy[]
): CopilotSdkUserInputPolicy {
  return async (ctx) => {
...
/**
 * Adapt a `CopilotSdkUserInputPolicy` to the SDK's `UserInputHandler`
...
export function createUserInputBridge(
  policy: CopilotSdkUserInputPolicy = denyAllUserInputPolicy,
): UserInputHandler {
  return async (
    request: SdkUserInputRequest,
    invocation: { sessionId: string },
  ): Promise<SdkUserInputResponse> => {
    const ctx: CopilotSdkUserInputContext = {
      request,
