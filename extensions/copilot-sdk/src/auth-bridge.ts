...
export const COPILOT_SDK_DEFAULT_AGENT_ID = "copilot-sdk";
...
/**
 * Resolve copilot-sdk auth + copilotHome.
 *
...
export function resolveCopilotAuth(input: ResolveCopilotAuthInput): ResolvedCopilotAuth {
  ...
  if (explicitToken) {
    if (!explicitProfileId || !explicitProfileVersion) {
      throw new Error(COPILOT_SDK_TOKEN_PROFILE_ERROR);
    }
    return {
 ...
 * (`COPILOT_SDK_DEFAULT_AGENT_ID`) rather than throwing - the harness's
 * job is to keep running with a safe default, not to validate config.
 */
export function sanitizeAgentId(value: string | undefined | null): string {
  const trimmed = (value ?? "").trim().toLowerCase();
  if (!trimmed) return COPILOT_SDK_DEFAULT_AGENT_ID;
  if (!/^[a-z0-9][a-z0-9_-]{0,63}$/.test(trimmed)) {
    return COPILOT_SDK_DEFAULT_AGENT_ID;
  }
  return trimmed;
}
