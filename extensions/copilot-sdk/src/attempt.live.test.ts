...
import { createCopilotSdkAgentHarness, type CopilotClientPool } from "../harness.js";

const liveToolState = vi.hoisted(() => ({
  calls: [] as string[],
  expectedText: "phase-1-green",
  sentinelPrefix: "copilot-sdk-live-smoke:",
  toolName: "live_echo",
}));
    ...
    createOpenClawCodingTools: vi.fn(() => [
      {
        ...
        description: "Echo the requested text for the copilot-sdk live smoke test.",
        parameters: {
          type: "object",
        ...
        async execute(_toolCallId: string, params: unknown) {
          ...
          console.info(
            `[copilot-sdk-live-smoke] ${liveToolState.toolName} ${JSON.stringify({ echoed, text })}`,
          );
          return {
...
const LIVE = isLiveTestEnabled(["OPENCLAW_COPILOT_SDK_LIVE_TEST"]);
const TOKEN =
  process.env.OPENCLAW_COPILOT_SDK_LIVE_TOKEN ||
...
function createAttemptParams(params: {
  ...
  return {
    ...
    agentId: "copilot-sdk-live-smoke",
    auth: {
      gitHubToken: TOKEN,
      profileId,
      profileVersion,
...
describeLive("copilot-sdk harness live smoke", () => {
  it("runs one turn on gpt-4.1 with one custom tool", async () => {
    ...
    const copilotHome = await mkdtemp(join(tmpdir(), "openclaw-copilot-sdk-live-"));
    const harness = createCopilotSdkAgentHarness({ pool: createApproveAllPool() });
    ...
    try {
      ...
      console.info(
        "[copilot-sdk-live-smoke] summary",
        JSON.stringify(
          {
            assistantText,
