...
export type CopilotSdkUsageSnapshot = NormalizedUsage;

function isCopilotSdkUsageSource(data: unknown): data is CopilotSdkUsageSource {
...
export function normalizeCopilotSdkUsage(data: unknown): NormalizedUsage | undefined {
  if (!isCopilotSdkUsageSource(data)) {
    return undefined;
  }
...
export function buildCopilotSdkAssistantUsage(params: {
  usage?: NormalizedUsage;
  fallbackOutputTokens?: unknown;
}): AssistantMessage["usage"] {
  const usage =
    params.usage ?? normalizeCopilotSdkUsage({ outputTokens: params.fallbackOutputTokens });
...
export function deriveCopilotSdkUsageTotal(usage?: NormalizedUsage): number | undefined {
  if (!usage) {
    return undefined;
  }
