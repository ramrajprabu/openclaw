...
describeLive("copilot-sdk harness /btw with real github-copilot auth profile (round-5 G2)", () => {
  it("loads the profile via getApiKeyForModel, forwards resolvedApiKey, and hits Copilot with that token", async () => {
    const agentDir = await mkdtemp(join(tmpdir(), "openclaw-copilot-sdk-btw-auth-"));
    ...
    try {
      ...
      const sideParams: AgentHarnessSideQuestionParams = {
        ...
        agentId: "copilot-sdk-btw-auth-live",
        workspaceDir: process.cwd(),
