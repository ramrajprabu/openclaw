...
import {
  COPILOT_SDK_DEFAULT_AGENT_ID,
  COPILOT_SDK_TOKEN_PROFILE_ERROR,
...
describe("sanitizeAgentId", () => {
  it("returns default for null/undefined/empty", () => {
    expect(sanitizeAgentId(undefined)).toBe(COPILOT_SDK_DEFAULT_AGENT_ID);
    expect(sanitizeAgentId(null)).toBe(COPILOT_SDK_DEFAULT_AGENT_ID);
    expect(sanitizeAgentId("")).toBe(COPILOT_SDK_DEFAULT_AGENT_ID);
    expect(sanitizeAgentId("   ")).toBe(COPILOT_SDK_DEFAULT_AGENT_ID);
  });
  ...
  it("rejects path-traversal segments and falls back to default", () => {
    expect(sanitizeAgentId("../etc/passwd")).toBe(COPILOT_SDK_DEFAULT_AGENT_ID);
    expect(sanitizeAgentId("../..")).toBe(COPILOT_SDK_DEFAULT_AGENT_ID);
    expect(sanitizeAgentId("a/b")).toBe(COPILOT_SDK_DEFAULT_AGENT_ID);
    expect(sanitizeAgentId("a\\b")).toBe(COPILOT_SDK_DEFAULT_AGENT_ID);
    expect(sanitizeAgentId("a\u0000b")).toBe(COPILOT_SDK_DEFAULT_AGENT_ID);
  });

  it("rejects ids that do not start with alnum", () => {
    expect(sanitizeAgentId("-foo")).toBe(COPILOT_SDK_DEFAULT_AGENT_ID);
    expect(sanitizeAgentId("_bar")).toBe(COPILOT_SDK_DEFAULT_AGENT_ID);
  });

  it("rejects ids longer than 64 chars", () => {
    expect(sanitizeAgentId("a".repeat(64))).toBe("a".repeat(64));
    expect(sanitizeAgentId("a".repeat(65))).toBe(COPILOT_SDK_DEFAULT_AGENT_ID);
  });
});
...
describe("resolveCopilotAuth - copilotHome resolution", () => {
  ...
  it("uses the default agent id when agentId is invalid/missing", () => {
    ...
    expect(result.agentId).toBe(COPILOT_SDK_DEFAULT_AGENT_ID);
    expect(result.copilotHome).toBe(
      resolve(join(FAKE_HOME, ".openclaw", "agents", COPILOT_SDK_DEFAULT_AGENT_ID, "copilot")),
    );
  });
...
describe("resolveCopilotAuth - auth mode resolution", () => {
  ...
  it("throws when explicit gitHubToken is given without both profileId + profileVersion", () => {
    ...
    ).toThrow(COPILOT_SDK_TOKEN_PROFILE_ERROR);
