/**
 * Permission bridge for the copilot-sdk harness.
 *
 ...
 *
 *   1. Defines a small `CopilotSdkPermissionPolicy` contract that the
 ...
 * `AgentHarnessAttemptParams` for the copilot-sdk harness.
 *
...
export interface CopilotSdkPermissionContext {
 ...
 */
export type CopilotSdkPermissionPolicy = (
  ctx: CopilotSdkPermissionContext,
...
export const REJECT_ALL_FEEDBACK =
  "copilot-sdk harness: no permission policy installed (fail-closed default)";

export const rejectAllPolicy: CopilotSdkPermissionPolicy = () => ({
  kind: "reject",
 ...
 */
export const allowOncePolicy: CopilotSdkPermissionPolicy = () => ({
  kind: "approve-once",
});
 ...
 */
export function allowListPolicy(options: AllowListPolicyOptions): CopilotSdkPermissionPolicy {
...
export interface DelegatingPolicyOptions {
   ...
   */
  onRequest: CopilotSdkPermissionPolicy;
  /**
   ...
   */
  onError?: CopilotSdkPermissionPolicy;
}
 ...
 */
export function delegatingPolicy(options: DelegatingPolicyOptions): CopilotSdkPermissionPolicy {
  ...
  return async (ctx) => {
    try {
      ...
      return {
        kind: "reject",
        feedback: `copilot-sdk permission policy threw: ${formatError(error)}`,
      };
    }
  };
}
...
export function composePolicies(
  ...policies: CopilotSdkPermissionPolicy[]
): CopilotSdkPermissionPolicy {
  return async (ctx) => {
...
/**
 * Adapt a `CopilotSdkPermissionPolicy` to the SDK's
...
export function createPermissionBridge(
  policy: CopilotSdkPermissionPolicy = rejectAllPolicy,
): PermissionHandler {
  return async (request, invocation) => {
    const ctx: CopilotSdkPermissionContext = {
      request,
