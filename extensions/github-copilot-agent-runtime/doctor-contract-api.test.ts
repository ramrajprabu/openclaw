...
describe("github-copilot doctor contract", () => {
  ...
  it("normalizeCompatibilityConfig is a structural no-op when no migrations apply", () => {
    const cfg = {
      plugins: {
        entries: { "github-copilot": { enabled: true, config: { pool: { idleTtlMs: 12345 } } } },
      },
  ...
  it("declares exactly one session route state owner for github-copilot", () => {
    expect(sessionRouteStateOwners).toHaveLength(1);
    const owner = sessionRouteStateOwners[0]!;
    expect(owner.id).toBe("github-copilot");
  ...
  it("claims the github-copilot runtime, session key, and auth profile prefix", () => {
    const owner = sessionRouteStateOwners[0]!;
    expect(owner.runtimeIds).toEqual(["github-copilot"]);
    expect(owner.cliSessionKeys).toEqual(["github-copilot"]);
