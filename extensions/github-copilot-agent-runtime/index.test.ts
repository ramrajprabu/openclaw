...
vi.mock("./harness.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    createGithubCopilotAgentHarness: vi.fn(actual.createGithubCopilotAgentHarness),
  };
});
...
describe("github-copilot plugin", () => {
  it("is opt-in by default and only declares an agent harness activation", () => {
    ...
    expect(activation.onAgentHarnesses).toEqual(["github-copilot"]);
    expect(manifest.providers).toBeUndefined();
    expect(typeof manifest.version).toBe("string");
    expect(manifest.version).not.toBe("");
  });

  it("registers exactly one github-copilot agent harness and nothing else", () => {
    const registerAgentHarness = vi.fn();
    ...
    expect(registerAgentHarness).toHaveBeenCalledWith(
      expect.objectContaining({ id: "github-copilot", label: "GitHub Copilot agent runtime" }),
    );
  ...
  it("passes through a valid pool idle TTL and ignores malformed values", () => {
    const createHarness = vi.mocked(createGithubCopilotAgentHarness);
