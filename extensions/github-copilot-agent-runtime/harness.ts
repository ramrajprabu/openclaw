...
const GITHUB_COPILOT_PROVIDER_IDS: ReadonlySet<string> = new Set(["github-copilot"]);

export interface CreateGithubCopilotAgentHarnessOptions {
  id?: string;
  label?: string;
...
function computeSessionCompatKey(params: AgentHarnessAttemptParams): string {
  ...
  // `runGithubCopilotAttempt`, so we don't want to mask it here — but
...
export function createGithubCopilotAgentHarness(
  options?: CreateGithubCopilotAgentHarnessOptions,
): AgentHarness {
  ...
  // runGithubCopilotAttempt via the onSessionEstablished callback so that
  ...
  return {
    ...
    supports(ctx) {
      const requestedRuntime = String(ctx.requestedRuntime ?? "")
        .trim()
        .toLowerCase();
      if (requestedRuntime !== "github-copilot") {
        return { supported: false, reason: "github-copilot is opt-in only" };
      }
      const provider = ctx.provider.trim().toLowerCase();
      if (!GITHUB_COPILOT_PROVIDER_IDS.has(provider)) {
        return {
          supported: false,
          reason: `provider is not one of: ${[...GITHUB_COPILOT_PROVIDER_IDS].toSorted().join(", ")}`,
        };
      }
    ...
    async runAttempt(params: AgentHarnessAttemptParams): Promise<AgentHarnessAttemptResult> {
      if (disposed) {
        throw new Error("[github-copilot] harness has been disposed; cannot start new attempts");
      }
      const { runGithubCopilotAttempt } = await import("./src/attempt.js");
      ...
      const attemptPromise = runGithubCopilotAttempt(effectiveParams, {
        pool,
    ...
    async runSideQuestion(
      ...
      if (disposed) {
        throw new Error("[github-copilot] harness has been disposed; cannot run side questions");
      }
      const { runGithubCopilotSideQuestion } = await import("./src/side-question.js");
      const pool = await getPool();
      const sidePromise = runGithubCopilotSideQuestion(params, { pool });
    ...
    async dispose() {
      ...
      disposePromise = (async () => {
        ...
        if (createdPool) {
          ...
          if (errors.length > 0) {
            throw new AggregateError(errors, "[github-copilot] pool disposal errors");
          }
        }
      })();
