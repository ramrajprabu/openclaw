/**
 * User-input bridge for the github-copilot agent runtime.
 *
 ...
 * module:
 *
 *   1. Defines a small `GithubCopilotUserInputPolicy` contract that the
...
export interface GithubCopilotUserInputContext {
 ...
 * code path.
 */
export type GithubCopilotUserInputPolicy = (
  ctx: GithubCopilotUserInputContext,
...
export const DENY_ALL_ANSWER =
  "[github-copilot agent runtime: no user-input policy installed; request declined]";

export const denyAllUserInputPolicy: GithubCopilotUserInputPolicy = () => ({
 ...
 */
export const firstChoicePolicy: GithubCopilotUserInputPolicy = ({ request }) => {
...
export function staticAnswerPolicy(options: StaticAnswerPolicyOptions): GithubCopilotUserInputPolicy {
...
export interface DelegatingUserInputPolicyOptions {
   ...
   */
  onRequest: GithubCopilotUserInputPolicy;
  /**
   ...
   */
  onError?: GithubCopilotUserInputPolicy;
}
...
export function composeUserInputPolicies(
  ...policies: GithubCopilotUserInputPolicy[]
): GithubCopilotUserInputPolicy {
  return async (ctx) => {
...
/**
 * Adapt a `GithubCopilotUserInputPolicy` to the SDK's `UserInputHandler`
...
export function createUserInputBridge(
  policy: GithubCopilotUserInputPolicy = denyAllUserInputPolicy,
): UserInputHandler {
  return async (
    request: SdkUserInputRequest,
    invocation: { sessionId: string },
  ): Promise<SdkUserInputResponse> => {
    const ctx: GithubCopilotUserInputContext = {
      request,
