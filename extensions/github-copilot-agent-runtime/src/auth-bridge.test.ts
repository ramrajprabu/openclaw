...
import {
  GITHUB_COPILOT_DEFAULT_AGENT_ID,
  GITHUB_COPILOT_TOKEN_PROFILE_ERROR,
...
describe("sanitizeAgentId", () => {
  it("returns default for null/undefined/empty", () => {
    expect(sanitizeAgentId(undefined)).toBe(GITHUB_COPILOT_DEFAULT_AGENT_ID);
    expect(sanitizeAgentId(null)).toBe(GITHUB_COPILOT_DEFAULT_AGENT_ID);
    expect(sanitizeAgentId("")).toBe(GITHUB_COPILOT_DEFAULT_AGENT_ID);
    expect(sanitizeAgentId("   ")).toBe(GITHUB_COPILOT_DEFAULT_AGENT_ID);
  });
  ...
  it("rejects path-traversal segments and falls back to default", () => {
    expect(sanitizeAgentId("../etc/passwd")).toBe(GITHUB_COPILOT_DEFAULT_AGENT_ID);
    expect(sanitizeAgentId("../..")).toBe(GITHUB_COPILOT_DEFAULT_AGENT_ID);
    expect(sanitizeAgentId("a/b")).toBe(GITHUB_COPILOT_DEFAULT_AGENT_ID);
    expect(sanitizeAgentId("a\\b")).toBe(GITHUB_COPILOT_DEFAULT_AGENT_ID);
    expect(sanitizeAgentId("a\u0000b")).toBe(GITHUB_COPILOT_DEFAULT_AGENT_ID);
  });

  it("rejects ids that do not start with alnum", () => {
    expect(sanitizeAgentId("-foo")).toBe(GITHUB_COPILOT_DEFAULT_AGENT_ID);
    expect(sanitizeAgentId("_bar")).toBe(GITHUB_COPILOT_DEFAULT_AGENT_ID);
  });

  it("rejects ids longer than 64 chars", () => {
    expect(sanitizeAgentId("a".repeat(64))).toBe("a".repeat(64));
    expect(sanitizeAgentId("a".repeat(65))).toBe(GITHUB_COPILOT_DEFAULT_AGENT_ID);
  });
});
...
describe("resolveCopilotAuth - copilotHome resolution", () => {
  ...
  it("uses the default agent id when agentId is invalid/missing", () => {
    ...
    expect(result.agentId).toBe(GITHUB_COPILOT_DEFAULT_AGENT_ID);
    expect(result.copilotHome).toBe(
      resolve(join(FAKE_HOME, ".openclaw", "agents", GITHUB_COPILOT_DEFAULT_AGENT_ID, "copilot")),
    );
  });
...
describe("resolveCopilotAuth - auth mode resolution", () => {
  ...
  it("throws when explicit gitHubToken is given without both profileId + profileVersion", () => {
    ...
    ).toThrow(GITHUB_COPILOT_TOKEN_PROFILE_ERROR);
