/**
 * Mirrors the AgentMessages produced by the github-copilot agent runtime into the
 ...
 * caller-side `dualWriteGithubCopilotTranscriptBestEffort` wrapper used
 ...
 */
export function attachGithubCopilotMirrorIdentity<T extends AgentMessage>(
  message: T,
  identity: string,
): T {
...
export interface MirrorGithubCopilotTranscriptParams {
  sessionFile: string;
  sessionKey?: string;
  agentId?: string;
   ...
   * `codex-app-server:${threadId}`; we use `github-copilot:${sessionId}`
...
export async function mirrorGithubCopilotTranscript(
  params: MirrorGithubCopilotTranscriptParams,
): Promise<void> {
 ...
 * this turn.
 */
export async function dualWriteGithubCopilotTranscriptBestEffort(
  params: MirrorGithubCopilotTranscriptParams,
): Promise<void> {
  try {
    await mirrorGithubCopilotTranscript(params);
  } catch (error) {
    console.warn("[github-copilot-attempt] dual-write transcript mirror failed", error);
  }
}
