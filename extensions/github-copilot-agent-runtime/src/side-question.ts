...
import {
  ...
  SUPPORTED_PROVIDERS,
  toError,
  type GithubCopilotPoolAcquireInput,
} from "./attempt.js";
...
type SideQuestionParamsLike = AgentHarnessSideQuestionParams & {
  auth?: GithubCopilotPoolAcquireInput["auth"];
  copilotHome?: string;
  cwd?: string;
...
function buildSideQuestionPoolInput(params: SideQuestionParamsLike): GithubCopilotPoolAcquireInput {
  ...
  } as unknown as GithubCopilotPoolAcquireInput;
}
...
export async function runGithubCopilotSideQuestion(
  ...
  if (!question) {
    throw new Error("[github-copilot-side-question] question is empty");
  }

  const modelRef = resolveModelRef(input as unknown as GithubCopilotPoolAcquireInput);
  if (!SUPPORTED_PROVIDERS.has(modelRef.provider)) {
    throw new Error(
      `[github-copilot-side-question] provider ${modelRef.provider} is not supported at MVP (subscription Copilot models only)`,
    );
  }
  ...
  try {
    ...
    if (result === undefined) {
      ...
      timedOut = true;
      primaryError = new Error(`[github-copilot-side-question] timed out after ${timeoutMs}ms`);
    }
  } catch (err: unknown) {
