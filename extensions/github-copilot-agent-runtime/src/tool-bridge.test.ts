...
describe("createGithubCopilotToolBridge", () => {
  it("returns empty arrays for unsupported providers without calling the seam", async () => {
    ...
    const result = await createGithubCopilotToolBridge({
      agentId: "agent-1",
  ...
  it("forwards supported fields to injected createOpenClawCodingTools", async () => {
    ...
    await createGithubCopilotToolBridge({
  ...
  // finding F6 and docs/plugins/github-copilot-agent-runtime.md.
...
describe("convertOpenClawToolToSdkTool", () => {
  ...
  it("returns a failure result when the signal is already aborted", async () => {
    ...
    expect(result).toMatchObject({
      resultType: "failure",
      textResultForLlm: "[github-copilot-tool-bridge] aborted before execution",
    });
    expect(getError(result as ToolResultObject)).toBe(
      "[github-copilot-tool-bridge] aborted before execution",
    );
  });
  ...
  it("returns a failure result when beforeExecute throws", async () => {
    ...
    expect(result).toMatchObject({
      resultType: "failure",
      textResultForLlm:
        "[github-copilot-tool-bridge] beforeExecute failed for tool 'tool-a': permission denied",
    });
  ...
  it("returns a failure result when prepareArguments throws", async () => {
    ...
    expect(result).toMatchObject({
      resultType: "failure",
      textResultForLlm:
        "[github-copilot-tool-bridge] prepareArguments failed for tool 'tool-a': bad args",
    });
    expect(getError(result as ToolResultObject)).toBe(error.message);
  });
  ...
  it("returns a failure result for unsupported content shapes", async () => {
    ...
    expect(result).toMatchObject({
      resultType: "failure",
      textResultForLlm:
        "[github-copilot-tool-bridge] unsupported AgentToolResult content shape: resource",
    });
    expect(getError(result as ToolResultObject)).toBe(
      "[github-copilot-tool-bridge] unsupported AgentToolResult content shape: resource",
    );
  ...
  it("returns a failure result when execute throws and preserves the error", async () => {
    ...
    expect(result).toMatchObject({
      resultType: "failure",
      textResultForLlm: "[github-copilot-tool-bridge] tool 'tool-a' failed: tool exploded",
    });
    expect(getError(result as ToolResultObject)).toBe(error.message);
  });
  ...
  it("returns a failure result when execute observes an abort after start", async () => {
    ...
    expect(result).toMatchObject({
      resultType: "failure",
      textResultForLlm: "[github-copilot-tool-bridge] tool 'tool-a' failed: aborted during execute",
    });
