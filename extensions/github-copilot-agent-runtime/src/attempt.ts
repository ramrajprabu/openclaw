...
import {
  ...
  type GithubCopilotInfiniteSessionOptions,
...
import { createHooksBridge, type GithubCopilotHooksConfig } from "./hooks-bridge.js";
import {
  createPermissionBridge,
  rejectAllPolicy,
  type GithubCopilotPermissionPolicy,
...
import { createGithubCopilotToolBridge } from "./tool-bridge.js";
...
// project openclaw-github-copilot-harness; reviewer-attempt-bridge note.
...
type AttemptParamsLike = AgentHarnessAttemptParams & {
  ...
  hooksConfig?: GithubCopilotHooksConfig;
  infiniteSessionConfig?: GithubCopilotInfiniteSessionOptions;
  ...
  permissionPolicy?: GithubCopilotPermissionPolicy;
  profileVersion?: string;
...
export type { AttemptParamsLike as GithubCopilotPoolAcquireInput, ModelRef };
export { SUPPORTED_PROVIDERS };

export interface GithubCopilotAttemptDeps {
  pool: CopilotClientPool;
  now?: () => number;
  createToolBridge?: typeof createGithubCopilotToolBridge;
  /**
...
export async function runGithubCopilotAttempt(
  params: AgentHarnessAttemptParams,
  deps: GithubCopilotAttemptDeps,
): Promise<AgentHarnessAttemptResult> {
  const now = deps.now ?? Date.now;
  const input = params as AttemptParamsLike;
  const createToolBridge = deps.createToolBridge ?? createGithubCopilotToolBridge;
  ...
  if (!SUPPORTED_PROVIDERS.has(modelRef.provider)) {
    return createResult(input, {
      ...
      promptError: createPromptError(
        "model_not_supported",
        `[github-copilot-attempt] provider ${modelRef.provider} is not supported at MVP (subscription Copilot models only; BYOK arrives via byok-mapping-skeleton)`,
      ),
      sdkSessionId: undefined,
      sessionIdUsed: input.sessionId,
    });
  ...
  // See tool-bridge.ts GithubCopilotSessionHolder.
  ...
  try {
    ...
    try {
      const toolBridge = await createToolBridge({
        ...
        modelId: modelRef.id,
        agentId: readString(params.agentId) ?? "github-copilot",
        sessionId: readString(input.sessionId) ?? "github-copilot-session",
      ...
      return createResult(input, {
        ...
        promptError: createPromptError(
          "tool_bridge_failure",
          `[github-copilot-attempt] tool-bridge construction failed: ${toError(error).message}`,
          error,
        ),
        sdkSessionId: undefined,
        sessionIdUsed: input.sessionId,
  ...
  // the user's prompt for a github-copilot attempt. The shell's
  ...
  const taggedLastAssistant = lastAssistant
    ? attachGithubCopilotMirrorIdentity(lastAssistant, `${input.runId}:assistant:final`)
    : undefined;
  ...
  if (sessionFileForMirror && messagesSnapshot.length > 0) {
    const taggedMessages = messagesSnapshot.map((message, index) => {
      ...
      return attachGithubCopilotMirrorIdentity(message, `${identityScope}:${message.role}:${index}`);
    });
    await dualWriteGithubCopilotTranscriptBestEffort({
      sessionFile: sessionFileForMirror,
      sessionKey: readString((input as { sessionKey?: unknown }).sessionKey),
      agentId: readString(input.agentId),
      messages: taggedMessages,
      idempotencyScope: sessionIdForScope ? `github-copilot:${sessionIdForScope}` : undefined,
      ...
      console.warn(
        "[github-copilot-attempt] dual-write transcript wrapper rejected unexpectedly",
        mirrorError,
      );
    });
  }
...
function createResult(
  ...
  return {
    ...
    sessionIdUsed: state.sessionIdUsed ?? readString(params.sessionId) ?? "github-copilot-session",
    timedOut,
...
> {
  ...
  return {
    ...
    // docs/plugins/github-copilot-agent-runtime.md.
