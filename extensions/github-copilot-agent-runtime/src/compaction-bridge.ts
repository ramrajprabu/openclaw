...
export type { SdkInfiniteSessionConfig as GithubCopilotInfiniteSessionConfig };

export interface GithubCopilotInfiniteSessionOptions {
  enabled?: boolean;
...
export function createInfiniteSessionConfig(
  options?: GithubCopilotInfiniteSessionOptions,
...
export interface OpenClawCompactionMarker {
  readonly version: 1;
  readonly source: "github-copilot-harness";
  readonly sessionId: string;
  readonly ts: number;
  /**
...
export async function writeOpenClawCompactionMarker(
  ...
  if (!input.workspaceDir || typeof input.workspaceDir !== "string") {
    throw new Error("[github-copilot:compaction-bridge] workspaceDir is required to write a marker");
  }
  if (!input.sessionId || typeof input.sessionId !== "string") {
    throw new Error("[github-copilot:compaction-bridge] sessionId is required to write a marker");
  }
  ...
  const marker: OpenClawCompactionMarker = compactJsonValue({
    version: 1 as const,
    source: "github-copilot-harness" as const,
