...
import { runGithubCopilotAttempt } from "./attempt.js";
...
const dualWriteMock = vi.hoisted(() => ({
  dualWriteGithubCopilotTranscriptBestEffort: vi.fn().mockResolvedValue(undefined),
  attachGithubCopilotMirrorIdentity: <T>(message: T, identity: string): T => {
...
describe("runGithubCopilotAttempt", () => {
  it("happy path", async () => {
    ...
    const result = await runGithubCopilotAttempt(makeParams(), { pool });
  ...
  it("subscribe-before-send", async () => {
    ...
    await runGithubCopilotAttempt(makeParams(), { pool });
  ...
  it("deltas forwarded in order via promise chain", async () => {
    ...
    const runPromise = runGithubCopilotAttempt(makeParams({ onAssistantDelta }), {
      createToolBridge,
      pool,
    });
    await flushAsync();
  ...
  it("deltas forwarded even when no consumer", async () => {
    ...
    const runPromise = runGithubCopilotAttempt(makeParams(), { createToolBridge, pool });
    await flushAsync();
  ...
  it("replay-shim: replayInvalid:true forces createSession even when sdkSessionId is present", async () => {
    ...
    const result = await runGithubCopilotAttempt(
      makeParams({
        initialReplayState: {
  ...
  it("abort path (mid-stream)", async () => {
    ...
    const runPromise = runGithubCopilotAttempt(makeParams({ abortSignal: controller.signal }), {
      createToolBridge,
      pool,
    });
    await flushAsync();
  ...
  it("abort path (signal already aborted)", async () => {
    ...
    const result = await runGithubCopilotAttempt(makeParams({ abortSignal: controller.signal }), {
      pool,
    });
  ...
  it("tool bridge wiring: injected tools populate session config", async () => {
    ...
    await runGithubCopilotAttempt(makeParams(), { createToolBridge, pool });
  ...
  it("F6: attemptParams carries the full input so the bridge can derive PI-parity tool context", async () => {
    ...
    await runGithubCopilotAttempt(params, { createToolBridge, pool });
  ...
  it("F7: result.yieldDetected is true when the tool bridge fires onYieldDetected during the attempt", async () => {
    ...
    const result = await runGithubCopilotAttempt(makeParams(), {
      createToolBridge,
      pool,
    });
  ...
  it("tool bridge failures become prompt errors", async () => {
    ...
    const result = await runGithubCopilotAttempt(makeParams(), { createToolBridge, pool });

    expect(getPromptErrorCode(result)).toBe("tool_bridge_failure");
    expect((result.promptError as Error | undefined)?.message).toBe(
      "[github-copilot-attempt] tool-bridge construction failed: bridge failed",
    );
    expect(sdk.createSession).toHaveBeenCalledTimes(0);
    expect(pool.acquire).toHaveBeenCalledTimes(0);
  ...
  it("enableSessionTelemetry: true is propagated to createSession", async () => {
    ...
    await runGithubCopilotAttempt(makeParams({ enableSessionTelemetry: true } as never), { pool });
  ...
  it("enableSessionTelemetry: false is propagated to createSession", async () => {
    ...
    await runGithubCopilotAttempt(makeParams({ enableSessionTelemetry: false } as never), { pool });
  ...
  it("infiniteSessions enabled:false explicitly disables infinite sessions", async () => {
    ...
    await runGithubCopilotAttempt(makeParams({ infiniteSessionConfig: { enabled: false } } as never), {
      pool,
    });
  ...
  it("release failure after a successful send rejects the attempt", async () => {
    ...
    await expect(runGithubCopilotAttempt(makeParams(), { pool })).rejects.toThrow("release failed");
  ...
  it("release failure after a primary prompt error warns without masking the error", async () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      "[github-copilot-attempt] pool.release failed after primary error",
  ...
  it("cleanup on disconnect throw", async () => {
    ...
    const first = await runGithubCopilotAttempt(makeParams(), { pool: poolWithPrimaryError });
    ...
    const second = await runGithubCopilotAttempt(makeParams(), { pool: poolWithoutPrimaryError });
  ...
  it("pool keying: gitHubToken requires profileId+profileVersion", async () => {
    ...
    await expect(
      runGithubCopilotAttempt(makeParams({ auth: { gitHubToken: "token" } as never }), { pool }),
    ).rejects.toThrow(
      "[github-copilot-attempt] gitHubToken auth requires profileId+profileVersion (pool keying safety; per Q5/Q1 decisions)",
    );
    expect(pool.acquire).toHaveBeenCalledTimes(0);
    expect(sdk.createSession).toHaveBeenCalledTimes(0);
  });
  ...
  describe("session-level gitHubToken (independent of client-level)", () => {
    ...
    it("SessionConfig.gitHubToken is omitted when default mode is useLoggedInUser (no auth signal)", async () => {
      ...
      try {
        await runGithubCopilotAttempt(makeParams({ auth: {} as never }), { pool });
        const cfg = sdk.createSession.mock.calls[0]?.[0] as Record<string, unknown>;
        expect("gitHubToken" in cfg).toBe(false);
      } finally {
  ...
  describe("dual-write transcript mirror", () => {
    afterEach(() => {
      dualWriteMock.dualWriteGithubCopilotTranscriptBestEffort.mockClear();
      dualWriteMock.dualWriteGithubCopilotTranscriptBestEffort.mockResolvedValue(undefined);
    });
    ...
    it("invokes dual-write mirror with sessionFile and scoped idempotencyScope when sessionFile is set", async () => {
      ...
      expect(dualWriteMock.dualWriteGithubCopilotTranscriptBestEffort).toHaveBeenCalledTimes(1);
      const args = dualWriteMock.dualWriteGithubCopilotTranscriptBestEffort.mock.calls[0]?.[0] as {
        sessionFile: string;
        messages: Array<{ role: string }>;
        idempotencyScope?: string;
      };
      expect(args.sessionFile).toBe("session.json");
      expect(args.idempotencyScope).toMatch(/^github-copilot:/u);
    ...
    it("does not invoke dual-write mirror when sessionFile is absent", async () => {
      ...
      await runGithubCopilotAttempt(params as never, { pool });

      expect(dualWriteMock.dualWriteGithubCopilotTranscriptBestEffort).not.toHaveBeenCalled();
    });

    it("tags mirrored messages with github-copilot mirror identity per role and position", async () => {
      dualWriteMock.dualWriteGithubCopilotTranscriptBestEffort.mockClear();
      const sdk = makeFakeSdk({
        onCreateSession: (session) => {
          session.sendAndWait.mockResolvedValueOnce(makeAssistantMessageEvent("done"));
    ...
    it("dual-write failure does not surface from runGithubCopilotAttempt", async () => {
      dualWriteMock.dualWriteGithubCopilotTranscriptBestEffort.mockRejectedValueOnce(
      ...
      // dualWriteGithubCopilotTranscriptBestEffort is already best-effort
      // internally; this test asserts attempt.ts also awaits it without
      // letting an unexpected rejection escape.
      await expect(runGithubCopilotAttempt(makeParams(), { pool })).resolves.toBeDefined();
    });
    ...
    it("injects synthetic user message with runId:prompt identity when caller passes no history", async () => {
      ...
      await runGithubCopilotAttempt(params, { pool });
    ...
    it("two attempts that share the same sdkSessionId but differ by runId produce distinct user/assistant mirror identities", async () => {
      ...
      const calls = dualWriteMock.dualWriteGithubCopilotTranscriptBestEffort.mock.calls;
