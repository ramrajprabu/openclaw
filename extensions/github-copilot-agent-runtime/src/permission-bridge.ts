/**
 * Permission bridge for the github-copilot agent runtime.
 *
 ...
 *
 *   1. Defines a small `GithubCopilotPermissionPolicy` contract that the
 ...
 * `AgentHarnessAttemptParams` for the github-copilot agent runtime.
 *
...
export interface GithubCopilotPermissionContext {
 ...
 */
export type GithubCopilotPermissionPolicy = (
  ctx: GithubCopilotPermissionContext,
...
export const REJECT_ALL_FEEDBACK =
  "github-copilot agent runtime: no permission policy installed (fail-closed default)";

export const rejectAllPolicy: GithubCopilotPermissionPolicy = () => ({
  kind: "reject",
 ...
 */
export const allowOncePolicy: GithubCopilotPermissionPolicy = () => ({
  kind: "approve-once",
});
 ...
 */
export function allowListPolicy(options: AllowListPolicyOptions): GithubCopilotPermissionPolicy {
...
export interface DelegatingPolicyOptions {
   ...
   */
  onRequest: GithubCopilotPermissionPolicy;
  /**
   ...
   */
  onError?: GithubCopilotPermissionPolicy;
}
 ...
 */
export function delegatingPolicy(options: DelegatingPolicyOptions): GithubCopilotPermissionPolicy {
  ...
  return async (ctx) => {
    try {
      ...
      return {
        kind: "reject",
        feedback: `github-copilot permission policy threw: ${formatError(error)}`,
      };
    }
  };
}
...
export function composePolicies(
  ...policies: GithubCopilotPermissionPolicy[]
): GithubCopilotPermissionPolicy {
  return async (ctx) => {
...
/**
 * Adapt a `GithubCopilotPermissionPolicy` to the SDK's
...
export function createPermissionBridge(
  policy: GithubCopilotPermissionPolicy = rejectAllPolicy,
): PermissionHandler {
  return async (request, invocation) => {
    const ctx: GithubCopilotPermissionContext = {
      request,
