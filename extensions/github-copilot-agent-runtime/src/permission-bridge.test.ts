...
import {
  ...
  rejectAllPolicy,
  REJECT_ALL_FEEDBACK,
  type GithubCopilotPermissionContext,
...
function makeCtx(
  overrides: Partial<GithubCopilotPermissionContext> = {},
): GithubCopilotPermissionContext {
  return {
    request: makeRequest(),
    sessionId: "sess-1",
    ...overrides,
...
describe("delegatingPolicy", () => {
  it("forwards the request to the host callback and returns its decision", async () => {
    const onRequest = vi.fn<GithubCopilotPermissionPolicy>().mockResolvedValue({
  ...
  it("returns the rejectAll default when host callback returns undefined", async () => {
    const onRequest = vi.fn<GithubCopilotPermissionPolicy>().mockResolvedValue(undefined);
    const policy = delegatingPolicy({ onRequest });
  ...
  it("rejects with the error message when host callback throws", async () => {
    const onRequest = vi
      .fn<GithubCopilotPermissionPolicy>()
  ...
  it("falls back to onError policy when host callback throws", async () => {
    const onError = vi.fn<GithubCopilotPermissionPolicy>().mockResolvedValue({ kind: "approve-once" });
...
describe("composePolicies", () => {
  it("returns the first non-undefined result and skips subsequent policies", async () => {
    const a: GithubCopilotPermissionPolicy = () => undefined;
    const b: GithubCopilotPermissionPolicy = () => ({ kind: "approve-once" });
    const c = vi.fn<GithubCopilotPermissionPolicy>(() => ({
      kind: "reject",
  ...
  it("short-circuits to reject if any policy throws (does not consult later policies)", async () => {
    const later = vi.fn<GithubCopilotPermissionPolicy>(() => ({ kind: "approve-once" }));
    const policy = composePolicies(() => {
...
describe("createPermissionBridge", () => {
  ...
  it("forwards the SDK sessionId into the policy context", async () => {
    const policy = vi.fn<GithubCopilotPermissionPolicy>(() => ({ kind: "approve-once" }));
