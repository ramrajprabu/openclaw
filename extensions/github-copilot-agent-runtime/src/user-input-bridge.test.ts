...
import {
  ...
  staticAnswerPolicy,
  DENY_ALL_ANSWER,
  type GithubCopilotUserInputContext,
  type GithubCopilotUserInputPolicy,
...
function makeCtx(overrides: Partial<GithubCopilotUserInputContext> = {}): GithubCopilotUserInputContext {
  return {
    request: makeRequest(),
    sessionId: "sess-1",
    ...overrides,
...
describe("delegatingUserInputPolicy", () => {
  it("forwards the request and returns the host response", async () => {
    const onRequest = vi
      .fn<GithubCopilotUserInputPolicy>()
  ...
  it("returns DENY_ALL_ANSWER when host callback returns undefined", async () => {
    const onRequest = vi.fn<GithubCopilotUserInputPolicy>().mockResolvedValue(undefined);
    const policy = delegatingUserInputPolicy({ onRequest });
    const result = await policy(makeCtx());
    expect(result).toEqual({ answer: DENY_ALL_ANSWER, wasFreeform: true });
  });
...
describe("composeUserInputPolicies", () => {
  it("returns the first non-undefined result and skips subsequent policies", async () => {
    const a: GithubCopilotUserInputPolicy = () => undefined;
    const b: GithubCopilotUserInputPolicy = () => ({ answer: "from-b", wasFreeform: true });
    const c = vi.fn<GithubCopilotUserInputPolicy>(() => ({ answer: "from-c", wasFreeform: true }));
  ...
  it("short-circuits to error-message response when any policy throws", async () => {
    const later = vi.fn<GithubCopilotUserInputPolicy>(() => ({ answer: "later", wasFreeform: true }));
    const policy = composeUserInputPolicies(() => {
...
describe("createUserInputBridge", () => {
  ...
  it("forwards the SDK sessionId into the policy context", async () => {
    const policy = vi.fn<GithubCopilotUserInputPolicy>(() => ({ answer: "x", wasFreeform: true }));
