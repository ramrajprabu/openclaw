...
import { createGithubCopilotAgentHarness, type CopilotClientPool } from "../harness.js";

const liveToolState = vi.hoisted(() => ({
  calls: [] as string[],
  expectedText: "phase-1-green",
  sentinelPrefix: "github-copilot-live-smoke:",
  toolName: "live_echo",
}));
    ...
    createOpenClawCodingTools: vi.fn(() => [
      {
        ...
        description: "Echo the requested text for the github-copilot live smoke test.",
        parameters: {
          type: "object",
        ...
        async execute(_toolCallId: string, params: unknown) {
          ...
          console.info(
            `[github-copilot-live-smoke] ${liveToolState.toolName} ${JSON.stringify({ echoed, text })}`,
          );
          return {
...
function createAttemptParams(params: {
  ...
  return {
    ...
    agentId: "github-copilot-live-smoke",
    auth: {
      gitHubToken: TOKEN,
      profileId,
      profileVersion,
    ...
    provider: "github-copilot",
    runId: `github-copilot-live-smoke-${now}`,
    sessionFile: "github-copilot-live-smoke.session.json",
    sessionId: `github-copilot-live-smoke-session-${now}`,
    timeoutMs: 90_000,
    workspaceDir: process.cwd(),
  } as unknown as AgentHarnessAttemptParams;
}

describeLive("github-copilot agent runtime live smoke", () => {
  it("runs one turn on gpt-4.1 with one custom tool", async () => {
    ...
    const copilotHome = await mkdtemp(join(tmpdir(), "openclaw-github-copilot-live-"));
    const harness = createGithubCopilotAgentHarness({ pool: createApproveAllPool() });
    ...
    try {
      ...
      console.info(
        "[github-copilot-live-smoke] summary",
        JSON.stringify(
          {
            assistantText,
