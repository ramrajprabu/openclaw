/**
 * Hooks bridge for the github-copilot agent runtime.
 *
...
export interface GithubCopilotHooksConfig {
...
const DEFAULT_HOOK_ERROR_HANDLER: NonNullable<GithubCopilotHooksConfig["onHookError"]> = ({
  hookName,
  error,
}) => {
  // eslint-disable-next-line no-console -- isolated diagnostic; subsystem logger lives outside the package boundary
  console.warn(`[github-copilot hooks-bridge] ${hookName} handler threw:`, error);
};
...
function isolate<TArgs extends readonly unknown[], TResult>(
  ...
  onError: NonNullable<GithubCopilotHooksConfig["onHookError"]>,
 ...
 * `GithubCopilotHooksConfig`. Returns `undefined` when no handlers were
 * supplied so the SDK skips the hook subsystem entirely.
 */
export function createHooksBridge(config?: GithubCopilotHooksConfig): SdkSessionHooks | undefined {
