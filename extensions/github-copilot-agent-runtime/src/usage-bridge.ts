...
export type GithubCopilotUsageSnapshot = NormalizedUsage;

function isGithubCopilotUsageSource(data: unknown): data is GithubCopilotUsageSource {
...
export function normalizeGithubCopilotUsage(data: unknown): NormalizedUsage | undefined {
  if (!isGithubCopilotUsageSource(data)) {
    return undefined;
  }
...
export function buildGithubCopilotAssistantUsage(params: {
  usage?: NormalizedUsage;
  fallbackOutputTokens?: unknown;
}): AssistantMessage["usage"] {
  const usage =
    params.usage ?? normalizeGithubCopilotUsage({ outputTokens: params.fallbackOutputTokens });
...
export function deriveGithubCopilotUsageTotal(usage?: NormalizedUsage): number | undefined {
  if (!usage) {
    return undefined;
  }
