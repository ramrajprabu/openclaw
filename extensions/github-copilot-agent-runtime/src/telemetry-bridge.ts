...
export type { SdkTraceContext as GithubCopilotTraceContext };
export type { SdkTelemetryConfig as GithubCopilotTelemetryConfig };

export type GithubCopilotTraceContextSource = () =>
  | SdkTraceContext
  | undefined
  | Promise<SdkTraceContext | undefined>;
export type GithubCopilotTraceparentSource = () => string | undefined | Promise<string | undefined>;
export type GithubCopilotTracestateSource = () => string | undefined | Promise<string | undefined>;

export interface GithubCopilotTraceContextErrorInfo {
  readonly part: "traceContext" | "traceparent" | "tracestate";
  readonly error: Error;
}

export interface GithubCopilotTraceContextOptions {
  /**
   ...
   */
  getTraceContext?: GithubCopilotTraceContextSource;
  /**
   ...
   */
  getTraceparent?: GithubCopilotTraceparentSource;
  /**
...
function defaultOnTraceContextError(info: GithubCopilotTraceContextErrorInfo): void {
  // eslint-disable-next-line no-console -- bridge has no host logger by design.
  console.warn(`[github-copilot:telemetry-bridge] ${info.part} source failed: ${info.error.message}`);
}

function safeNotify(
  notifier: (info: GithubCopilotTraceContextErrorInfo) => void,
  info: GithubCopilotTraceContextErrorInfo,
): void {
  try {
    notifier(info);
  } catch {
...
export function createTraceContextProvider(
  options?: GithubCopilotTraceContextOptions,
...
export interface GithubCopilotTelemetryOptions {
  otlpEndpoint?: string;
  filePath?: string;
  exporterType?: string;
  sourceName?: string;
...
export function createTelemetryConfig(
  options?: GithubCopilotTelemetryOptions,
