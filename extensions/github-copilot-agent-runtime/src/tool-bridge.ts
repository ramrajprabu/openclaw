 ...
 */
export interface GithubCopilotSessionHolder {
 ...
 */
export type GithubCopilotToolAttemptParams = Partial<EmbeddedRunAttemptParams>;

export interface GithubCopilotToolBridgeInput {
   ...
   */
  attemptParams?: GithubCopilotToolAttemptParams;
  /**
   * Mutable session holder used to wire `onYield` to the live
   * `session.abort()` once the SDK session is established. See
   * {@link GithubCopilotSessionHolder}.
   */
  sessionRef?: GithubCopilotSessionHolder;
  /**
...
export interface GithubCopilotToolBridge {
  sdkTools: SdkTool[];
...
export async function createGithubCopilotToolBridge(
  input: GithubCopilotToolBridgeInput,
): Promise<GithubCopilotToolBridge> {
  ...
  try {
    ...
    throw createError(
      `[github-copilot-tool-bridge] createOpenClawCodingTools failed: ${toError(error).message}`,
      error,
    );
  }
  ...
  if (!Array.isArray(sourceTools)) {
    throw new Error(
      "[github-copilot-tool-bridge] createOpenClawCodingTools must return an array of tools",
    );
  }

  const duplicateNames = findDuplicateToolNames(sourceTools as AnyAgentTool[]);
  if (duplicateNames.length > 0) {
    throw new Error(`[github-copilot-tool-bridge] duplicate tool names: ${duplicateNames.join(", ")}`);
  }
 ...
 * fields silently degrade policy decisions. See docs/plugins/github-copilot-agent-runtime.md.
 *
 ...
 * the github-copilot agent runtime does not currently route through
...
function buildOpenClawCodingToolsOptions(
  ...
  const a = input.attemptParams ?? ({} as GithubCopilotToolAttemptParams);
  ...
  return {
    ...
    // sandbox: undefined — github-copilot agent runtime does not route through
    ...
    // recordToolPrepStage intentionally omitted: github-copilot does not
    ...
    onYield: (message) => {
      ...
      try {
        ...
        console.warn(
          "[github-copilot-tool-bridge] onYieldDetected handler threw; continuing",
          error,
        );
      }
...
export function convertOpenClawToolToSdkTool(
  ...
  ctx: {
    abortSignal?: AbortSignal;
    beforeExecute?: GithubCopilotToolBridgeInput["beforeExecute"];
  },
): SdkTool {
  if (typeof sourceTool.name !== "string" || sourceTool.name.trim().length === 0) {
    throw new Error("[github-copilot-tool-bridge] tool name must be a non-empty string");
  }

  if (typeof sourceTool.execute !== "function") {
    throw new Error(
      `[github-copilot-tool-bridge] tool '${sourceTool.name}' must define an execute function`,
    );
  }
  ...
  const executeOnce = async (
    ...
    if (ctx.abortSignal?.aborted) {
      const error = new Error("[github-copilot-tool-bridge] aborted before execution");
    ...
    try {
      ...
      return createFailureResult(
        `[github-copilot-tool-bridge] beforeExecute failed for tool '${sourceTool.name}': ${toError(error).message}`,
        error,
      );
    }
    ...
    try {
      ...
      return createFailureResult(
        `[github-copilot-tool-bridge] prepareArguments failed for tool '${sourceTool.name}': ${toError(error).message}`,
        error,
      );
    }
    ...
    try {
      ...
      return createFailureResult(
        `[github-copilot-tool-bridge] tool '${sourceTool.name}' failed: ${toError(error).message}`,
        error,
      );
    }
  ...
  return {
    ...
    // names within a github-copilot attempt.
...
function createUnsupportedContentFailure(kind: string): ToolResultObject {
  const message = `[github-copilot-tool-bridge] unsupported AgentToolResult content shape: ${kind}`;
