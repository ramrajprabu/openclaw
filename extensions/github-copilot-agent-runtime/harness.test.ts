...
const mocks = vi.hoisted(() => ({
  runGithubCopilotAttempt: vi.fn(),
  runGithubCopilotSideQuestion: vi.fn(),
  createCopilotClientPool: vi.fn(),
}));

vi.mock("./src/attempt.js", () => ({
  runGithubCopilotAttempt: mocks.runGithubCopilotAttempt,
}));

vi.mock("./src/side-question.js", () => ({
  runGithubCopilotSideQuestion: mocks.runGithubCopilotSideQuestion,
}));
...
describe("createGithubCopilotAgentHarness", () => {
  beforeEach(() => {
    mocks.runGithubCopilotAttempt.mockReset();
    mocks.runGithubCopilotSideQuestion.mockReset();
    mocks.createCopilotClientPool.mockReset();
    mocks.runGithubCopilotAttempt.mockResolvedValue(ATTEMPT_RESULT);
    mocks.runGithubCopilotSideQuestion.mockResolvedValue({ text: "answer" });
    mocks.createCopilotClientPool.mockImplementation(() => makePoolMock());
  });

  it("returns the github-copilot id and default label", () => {
    const harness = createGithubCopilotAgentHarness();

    expect(harness.id).toBe("github-copilot");
    expect(harness.label).toBe("GitHub Copilot agent runtime");
  });

  it("accepts custom id and label from options", () => {
    const harness = createGithubCopilotAgentHarness({ id: "sdk", label: "SDK Harness" });
  ...
  it("supports returns false in auto runtime even for github provider", () => {
    ...
    expect(
      ...
      supported: false,
      reason: "github-copilot is opt-in only",
    });
  });
  ...
  it("supports returns true for requestedRuntime github-copilot with github-copilot provider", () => {
    const harness = createGithubCopilotAgentHarness();
    ...
    expect(
      harness.supports({
        ...
        modelId: "gpt-4.1",
        requestedRuntime: "github-copilot",
      }),
  ...
  it("supports normalizes requestedRuntime casing", () => {
    ...
    expect(
      harness.supports({
        provider: "github-copilot",
        modelId: "gpt-4.1",
        requestedRuntime: "  GITHUB-COPILOT  " as any,
      }),
    ).toEqual({ supported: true, priority: 100 });
  });
  ...
  it("runAttempt lazy-imports attempt by waiting until invocation to create a pool", async () => {
    ...
    expect(mocks.runGithubCopilotAttempt).not.toHaveBeenCalled();
    ...
    expect(mocks.runGithubCopilotAttempt).toHaveBeenCalledTimes(1);
  });
  ...
  it("runAttempt creates one pool lazily and reuses it across two attempts on the same harness", async () => {
    ...
    expect(mocks.createCopilotClientPool).toHaveBeenCalledTimes(1);
    expect(mocks.runGithubCopilotAttempt).toHaveBeenNthCalledWith(
      1,
      ATTEMPT_PARAMS,
  ...
  it("multiple harness instances create independent pools", async () => {
    ...
    const firstHarness = createGithubCopilotAgentHarness();
    const secondHarness = createGithubCopilotAgentHarness();
  ...
  it("runAttempt does not serialize concurrent attempts", async () => {
    ...
    expect(mocks.createCopilotClientPool).toHaveBeenCalledTimes(1);
    expect(mocks.runGithubCopilotAttempt).toHaveBeenCalledTimes(2);
  });
  ...
  it("dispose waits for in-flight runAttempt before disposing", async () => {
    ...
    mocks.createCopilotClientPool.mockReturnValue(pool);
    mocks.runGithubCopilotAttempt.mockImplementation(() => deferred.promise);
    const harness = createGithubCopilotAgentHarness();
  ...
  it("runAttempt after dispose rejects without creating a new pool", async () => {
    ...
    await expect(harness.runAttempt(ATTEMPT_PARAMS)).rejects.toThrow(
      "[github-copilot] harness has been disposed; cannot start new attempts",
    );
    expect(mocks.createCopilotClientPool).not.toHaveBeenCalled();
  });
  ...
  it("dispose surfaces pool.dispose errors as AggregateError", async () => {
    ...
    try {
      ...
      expect((error as AggregateError).message).toBe("[github-copilot] pool disposal errors");
  ...
  it("uses options.pool when supplied", async () => {
    ...
    expect(mocks.createCopilotClientPool).not.toHaveBeenCalled();
    expect(mocks.runGithubCopilotAttempt).toHaveBeenCalledWith(
      ATTEMPT_PARAMS,
      expect.objectContaining({ pool }),
    );
  });
  ...
  describe("reset", () => {
    ...
    it("calls deleteSession on the client that created the session", async () => {
      ...
      mocks.runGithubCopilotAttempt.mockImplementation(async (params, deps) => {
    ...
    it("does not call deleteSession when no sdkSessionId was reported", async () => {
      const pool = makePoolMock();
      const deleteSession = vi.fn().mockResolvedValue(undefined);
      mocks.runGithubCopilotAttempt.mockImplementation(async (_params, _deps) => ATTEMPT_RESULT);
      const harness = createGithubCopilotAgentHarness({ pool });
  ...
  describe("session reuse across turns (dogfood finding #4)", () => {
    ...
    it("seeds initialReplayState.sdkSessionId from trackedSessions on the second turn", async () => {
      ...
      mocks.runGithubCopilotAttempt.mockImplementation(async (_params, deps) => {
        deps.onSessionEstablished?.({
      ...
      expect(mocks.runGithubCopilotAttempt).toHaveBeenCalledTimes(2);
      const secondCallParams = mocks.runGithubCopilotAttempt.mock.calls[1]?.[0] as {
    ...
    it("does not seed sdkSessionId on the first turn (nothing tracked yet)", async () => {
      ...
      const firstCallParams = mocks.runGithubCopilotAttempt.mock.calls[0]?.[0] as {
  ...
  describe("compact", () => {
    it("returns ok:false when sessionId is missing", async () => {
      const harness = createGithubCopilotAgentHarness({ pool: makePoolMock() });
    ...
    it("writes an OpenClaw marker under <workspaceDir>/files and returns ok:true,compacted:false", async () => {
      const workspaceDir = await mkdtemp(join(tmpdir(), "github-copilot-harness-compact-"));
      try {
        const harness = createGithubCopilotAgentHarness({ pool: makePoolMock() });
    ...
    it("records the tracked sdkSessionId in the marker when an attempt has run", async () => {
      const workspaceDir = await mkdtemp(join(tmpdir(), "github-copilot-harness-compact-tracked-"));
      try {
        const pool = makePoolMock();
        mocks.runGithubCopilotAttempt.mockImplementation(async (params, deps) => {
          deps.onSessionEstablished?.({
    ...
    it("records force:true in the marker and surfaces a force-specific reason", async () => {
      const workspaceDir = await mkdtemp(join(tmpdir(), "github-copilot-harness-compact-force-"));
      try {
        const harness = createGithubCopilotAgentHarness({ pool: makePoolMock() });
        const result = await harness.compact?.({
  ...
  describe("runSideQuestion", () => {
    it("delegates to runGithubCopilotSideQuestion with the pool", async () => {
      const pool = makePoolMock();
      const harness = createGithubCopilotAgentHarness({ pool });
      ...
      expect(mocks.runGithubCopilotSideQuestion).toHaveBeenCalledTimes(1);
      expect(mocks.runGithubCopilotSideQuestion).toHaveBeenCalledWith(
        params,
        expect.objectContaining({ pool }),
      );
    });
    ...
    it("throws when called after dispose", async () => {
      ...
      expect(mocks.runGithubCopilotSideQuestion).not.toHaveBeenCalled();
    });

    it("propagates the side-question error to the caller", async () => {
      const err = new Error("boom");
      mocks.runGithubCopilotSideQuestion.mockRejectedValueOnce(err);
      const harness = createGithubCopilotAgentHarness({ pool: makePoolMock() });
    ...
    it("dispose waits for in-flight side questions before disposing the pool", async () => {
      ...
      mocks.runGithubCopilotSideQuestion.mockReturnValueOnce(deferred.promise);
      const harness = createGithubCopilotAgentHarness({ pool });
