...
import { extractAssistantText, runCopilotSideQuestion } from "./side-question.js";
...
describe("runCopilotSideQuestion", () => {
  ...
  it("throws when provider is not supported", async () => {
    const pool = makeFakePool(makeFakeClient(makeFakeSession()));
    await expect(
      runCopilotSideQuestion(makeParams({ provider: "anthropic" }), { pool: pool as any }),
  ...
  it("returns extracted assistant text on the happy path", async () => {
    ...
    const result = await runCopilotSideQuestion(makeParams(), { pool: pool as any });
  ...
  it("forwards a custom timeoutMs from deps", async () => {
    ...
    await runCopilotSideQuestion(makeParams(), { pool: pool as any, timeoutMs: 5_000 });
  ...
  it("times out, aborts the session, then disconnects and deletes", async () => {
    ...
    await expect(
      runCopilotSideQuestion(makeParams(), { pool: pool as any, timeoutMs: 1_000 }),
  ...
  it("G1: SDK timeout REJECTION ('Timeout after Nms waiting for session.idle') aborts the session, disconnects, deletes, and surfaces the canonical side-question timeout error", async () => {
    ...
    await expect(
      runCopilotSideQuestion(makeParams(), { pool: pool as any, timeoutMs: 1_500 }),
  ...
  it("does not call abort on a normal completion", async () => {
    const session = makeFakeSession();
    const pool = makeFakePool(makeFakeClient(session));
    await runCopilotSideQuestion(makeParams(), { pool: pool as any });
  ...
  it("propagates sendAndWait errors and still disconnects + deletes + releases", async () => {
    ...
    await expect(runCopilotSideQuestion(makeParams(), { pool: pool as any })).rejects.toBe(
      error,
    );
  ...
  it("propagates createSession failures and still releases the pool handle", async () => {
    ...
    await expect(runCopilotSideQuestion(makeParams(), { pool: pool as any })).rejects.toThrow(
  ...
  it("does not pass an SDK reasoningEffort (OpenClaw ReasoningLevel is not aligned)", async () => {
    ...
    await runCopilotSideQuestion(makeParams({ resolvedReasoningLevel: "on" }), {
      pool: pool as any,
    });
  ...
  it("trims whitespace from the question before sending", async () => {
    ...
    await runCopilotSideQuestion(makeParams({ question: "  hello?  " }), { pool: pool as any });
