...
describe("runBtwSideQuestion", () => {
  ...
  it("G2: forwards resolvedApiKey to the copilot agent runtime side-question hook (production /btw auth)", async () => {
    // The copilot agent runtime needs the resolved auth-profile token
    ...
    // getApiKeyForModel, so headless `/btw` from a copilot
    // profile silently fell back to env / useLoggedInUser.
    const copilotSideQuestionMock = vi
      .fn()
      .mockResolvedValue({ text: "copilot side answer." });
    registerAgentHarness({
      id: "copilot",
      label: "GitHub Copilot agent runtime test harness",
      supports: () => ({ supported: true, priority: 100 }),
      runAttempt: vi.fn(),
      runSideQuestion: copilotSideQuestionMock,
    });
    resolveModelWithRegistryMock.mockReturnValue({
    ...
    expect(result).toEqual({ text: "copilot side answer." });
    ...
    expect(copilotSideQuestionMock).toHaveBeenCalledTimes(1);
    const [[sideQuestionParams]] = copilotSideQuestionMock.mock.calls as unknown as Array<
      [
        {
          authProfileId?: string;
  ...
  it("G2: does NOT call getApiKeyForModel or forward resolvedApiKey for non-copilot agent runtimes (credential boundary stays narrow)", async () => {
  ...
  it("G2: copilot side question still runs (without resolvedApiKey) when getApiKeyForModel rejects", async () => {
    ...
    const copilotSideQuestionMock = vi.fn().mockResolvedValue({ text: "fallback answer." });
    registerAgentHarness({
      id: "copilot",
      label: "GitHub Copilot agent runtime test harness",
      supports: () => ({ supported: true, priority: 100 }),
